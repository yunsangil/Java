...
public class create_business_log {
	...
	void create_logic_inputdate() {
		...
		System.out.println("현재 입력하신 날짜는" + add + "입니다. 맞습니까?"); //business_log의 기본 toString이 year,month,day를 반환하므로 사용가능
		...
		if (input_select == 1) {
			...
			while (flag) {
				...
				if (input_select == 1) {
					...
					add.setLimit_year(input_year);     //business_log파일에도 설명이 있지만 setter가 따로 있는 이유는 실행 방식이 이렇게 설정할 수 밖에 없기 때문이다.
					...
					System.out.println(add + "에 " +"업무 번호 : " +WorkNum+ "업무명 : " + add.getWork() + "이 정상 기록되었습니다!");
					limit_date = (add.getLimit_year() * 365 + add.getLimit_month() * 30 + add.getLimit_day())
							- (add.getLog_year() * 365 + add.getLog_month() * 30 + add.getLog_day());
					System.out.println("만료일은 " + add.toString2() + "이고, 시작일 기준으로 남은 일수는" + limit_date + "일 입니다. 숙지해주세요!");
